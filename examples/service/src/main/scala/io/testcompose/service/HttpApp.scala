package io.testcompose.service

import _root_.smithy4s.http4s.*
import fs2.io.net.Network
import io.poc.twilio.config.AppConfig
...
object HttpApp {
  ...
  private val serviceRoutesResource = for {
    twilioService <- ZIO.service[smithy.TwilioService[Task]]
    twilioRoutes <- SimpleRestJsonBuilder
      .routes(twilioService)
      .resource
      .toScopedZIO
  } yield twilioRoutes
  ...
  val serverLayer = ZLayer.scoped {
    (for {
      ...
      servers <- server(config.health, healthRoutes)
        &> server(config.service, serviceRoutes)
